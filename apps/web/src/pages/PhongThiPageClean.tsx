...
export function PhongThiPageClean() {
  ...
  return (
    <div className="stack exam-shell no-select-zone">
      ...
      <div className="card hero-panel dashboard-gradient-admin">
        ...
        <p>Câu hỏi hiển thị rõ ràng, đáp án A/B/C/D dễ bấm, có đồng hồ đếm ngược và các cảnh báo chống gian lận cần thiết.</p>
        ...
        <div className="grid-2 lesson-layout">
          <div className="card stack">
            <div className="row-between wrap-mobile">
              ...
              <span className="badge badge-soft">{currentQuestion.mucDo}</span>
            </div>
            <div className="markdown-box">{currentQuestion.noiDung}</div>
            <div className="stack">
              {(currentQuestion.options || []).map((choice: { key: string; text: string }) => {
                ...
                return (
                  ...
                  >
                    <span className="exam-option-index">{choice.key}</span>
      ...
      {submitted && (
        <div className="card stack">
          ...
          {Array.isArray(submitted.chiTietJson?.studentResult?.details) && submitted.chiTietJson.studentResult.details.length ? (
            ...
            <div className="note-box">Giáo viên đang bật chế độ ẩn đáp án chi tiết sau khi nộp bài.</div>
          )}
        </div>
      )}
    </div>
