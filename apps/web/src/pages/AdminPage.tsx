...
function levelClass(level?: string) {
  return level === 'ERROR' ? 'status-pill danger' : level === 'WARN' ? 'status-pill warning' : 'status-pill success';
}
      ...
      <div className="grid-2">
        ...
        <div className="stack">
          <div>
            ...
            <select className="select" value={selectedType} onChange={(e) => {
              ...
              onChange({
                ...
                moPhong: {
                  ...(lesson.moPhong || {}),
                  loaiMoPhong: nextType,
                  thamSoJson: preset?.defaultParams || {},
                  cauHinhJson: { ...config, topic: lesson.chuDeThi || config.topic || 'Tổng hợp', renderer: 'kntech-threejs-full3d', sceneVariant: lesson.slug || lesson.id, allowExternalResources: true }
                }
              });
            }}>
              <option value="">Chọn simulation type</option>
              {simulationLibrary.map((item) => <option key={item.type} value={item.type}>{item.group} · {item.label}</option>)}
            </select>
            {selectedPreset && <div className="note-box">{selectedPreset.summary}</div>}
          </div>
          ...
          <div className="stack">
            ...
            <input className="input" value={String(config.imageUrl || '')} onChange={(e) => onChange({ ...lesson, moPhong: { ...(lesson.moPhong || {}), cauHinhJson: { ...config, imageUrl: e.target.value, allowExternalResources: true } } })} placeholder="https://... hoặc /uploads/..." />
      ...
      {sections.map((section: any, idx: number) => (
        <div key={section.id || idx} className="card section-card stack">
          <div className="grid-3 compact-filter-grid">
            ...
            <select className="select" value={section.mucDo || 'DE'} onChange={(e) => onChange({ ...lesson, phanKienThuc: sections.map((item: any, i: number) => i === idx ? { ...item, mucDo: e.target.value } : item) })}><option value="DE">DE</option><option value="TRUNG_BINH">TRUNG_BINH</option><option value="KHO">KHO</option></select>
  ...
  return (
    <div className="stack">
      ...
      {tab === 'tong-quan' && (
        <div className="stack">
          <div className="grid-2">
            <div className="card stack"><div className="row-between"><h3>Lesson coverage theo lớp</h3><span className="badge">KNTech Analytics</span></div><AreaTrend items={gradeTrend} /></div>
            <div className="card stack"><div className="row-between"><h3>AI providers</h3><span className="badge badge-soft">Live usage</span></div><AreaTrend items={aiTrend.length ? aiTrend : [{ label: 'LOCAL', value: 0 }]} /></div>
          </div>
          ...
          <div className="grid-2">
            <div className="card stack"><h3>Database runtime</h3><div className="table-wrap"><table className="table"><tbody><tr><td>Driver</td><td>MySQL</td></tr><tr><td>Host</td><td>{data.dbInfo.host}</td></tr><tr><td>Port</td><td>{data.dbInfo.port}</td></tr><tr><td>Database</td><td>{data.dbInfo.database}</td></tr><tr><td>User</td><td>{data.dbInfo.user}</td></tr></tbody></table></div></div>
            <div className="card stack"><h3>System monitor</h3><div className="table-wrap"><table className="table"><tbody><tr><td>Node</td><td>{data.runtime.node}</td></tr><tr><td>Platform</td><td>{data.runtime.platform}</td></tr><tr><td>Uptime</td><td>{data.runtime.uptimeHum}</td></tr><tr><td>RSS Memory</td><td>{data.runtime.memory.rssMb} MB</td></tr><tr><td>Heap Used</td><td>{data.runtime.memory.heapUsedMb} MB</td></tr><tr><td>CPU cores</td><td>{data.runtime.cpuCount}</td></tr></tbody></table></div></div>
          </div>
        </div>
      )}
          ...
          {Object.entries(groupedSettings).map(([group, items]: any) => (
            <div key={group} className="stack">
              ...
              <div className="settings-grid">
                {items.map((item: Setting) => <div key={item.ma} className="setting-card"><label className="label">{item.ten}</label><input className="input" value={String(item.giaTri ?? '')} onChange={(e) => setData((prev: any) => ({ ...prev, settings: prev.settings.map((s: Setting) => s.ma === item.ma ? { ...s, giaTri: e.target.value } : s) }))} /><div className="muted">{item.ma} · {item.moTa}</div></div>)}
              </div>
            </div>
          ))}
        </div>
      ...
      {tab === 'bai-hoc' && (
        <div className="grid-2">
          <div className="card stack">
            ...
            <div className="table-wrap"><table className="table compact-table"><thead><tr><th>Bài</th><th>Chủ đề</th><th>Tag lớp</th></tr></thead><tbody>{filteredLessons.map((lesson: any) => <tr key={lesson.id} className={selectedLessonId === lesson.id ? 'row-selected' : ''} onClick={() => setSelectedLessonId(lesson.id)}><td>{lesson.ten}</td><td>{lesson.moPhong?.cauHinhJson?.topic || lesson.chuong.ten}</td><td>Lớp {lesson.chuong.lop}</td></tr>)}</tbody></table></div>
          </div>
          <div className="stack"><div className="card stack"><div className="row-between"><h3>{selectedLesson?.ten || 'Chọn một bài học'}</h3><button className="button" onClick={saveLesson}>Lưu lesson</button></div>{selectedLesson ? <div className="stack"><SimulationEditor lesson={selectedLesson} onChange={(next) => setData((prev: any) => ({ ...prev, lessons: prev.lessons.map((item: any) => item.id === next.id ? next : item) }))} onUploadImage={uploadMediaForLesson} /><LessonContentEditor lesson={selectedLesson} onChange={(next) => setData((prev: any) => ({ ...prev, lessons: prev.lessons.map((item: any) => item.id === next.id ? next : item) }))} /></div> : <div className="note-box">Chọn một bài trong danh sách trái để biên tập.</div>}</div></div>
        </div>
      )}

      {tab === 'media' && (
        <div className="grid-2">
          <div className="card stack"><div className="row-between"><h3>Media manager</h3><span className="badge">KNTech Library</span></div><input type="file" className="input" onChange={(e) => uploadMedia(e.target.files?.[0])} disabled={uploading} /><div className="muted">Upload ảnh, video ngắn, mô hình hoặc tài nguyên hỗ trợ cho lesson builder.</div></div>
          <div className="card stack"><h3>Tệp gần đây</h3><div className="table-wrap"><table className="table compact-table"><thead><tr><th>Tên tệp</th><th>Size</th><th>Cập nhật</th></tr></thead><tbody>{data.media?.map((file: any) => <tr key={file.name}><td><a href={file.url} target="_blank" rel="noreferrer">{file.name}</a></td><td>{file.sizeMb} MB</td><td>{new Date(file.updatedAt).toLocaleString()}</td></tr>)}</tbody></table></div></div>
        </div>
      )}

      {tab === 'monitor' && (
        <div className="stack">
          <div className="card stack"><div className="row-between"><h3>API monitor</h3><span className="badge">{data.apiMetrics.totalHits} hits</span></div><AreaTrend items={logTrend.length ? logTrend : [{ label: 'api', value: 0 }]} /><div className="table-wrap"><table className="table compact-table"><thead><tr><th>Route</th><th>Hits</th><th>Avg ms</th><th>Errors</th><th>Status</th></tr></thead><tbody>{data.apiMetrics.routes.map((route: any) => <tr key={`${route.method}-${route.path}`}><td>{route.method} {route.path}</td><td>{route.hits}</td><td>{route.avgMs}</td><td>{route.errors}</td><td>{route.lastStatus}</td></tr>)}</tbody></table></div></div>
          <div className="card stack"><h3>System logs</h3><div className="table-wrap"><table className="table compact-table"><thead><tr><th>Thời gian</th><th>Mức</th><th>Nhóm</th><th>Hành động</th><th>Đối tượng</th><th>Người dùng</th></tr></thead><tbody>{data.logs.map((log: any) => <tr key={log.id}><td>{new Date(log.createdAt).toLocaleString()}</td><td><span className={levelClass(log.muc)}>{log.muc}</span></td><td>{log.nhom}</td><td>{log.hanhDong}</td><td>{log.doiTuong || '-'}</td><td>{log.nguoiDung?.email || '-'}</td></tr>)}</tbody></table></div></div>
        </div>
      )}

      {tab === 'ai' && (
        <div className="card stack"><h3>AI call logs</h3><div className="table-wrap"><table className="table"><thead><tr><th>Provider</th><th>Tác vụ</th><th>Prompt</th><th>Kết quả</th><th>Thời gian</th></tr></thead><tbody>{data.aiLogs.map((log: any) => <tr key={log.id}><td><span className={String(log.nhaCungCap).toLowerCase() === 'local' ? 'status-pill warning' : 'status-pill success'}>{String(log.nhaCungCap).toUpperCase()}</span></td><td>{log.loaiTacVu}</td><td>{log.promptRutGon}</td><td>{log.ketQuaRutGon}</td><td>{new Date(log.createdAt).toLocaleString()}</td></tr>)}</tbody></table></div></div>
      )}
      {tab === 'nguoi-dung' && (
        <div className="grid-2">
          <div className="stack">
            <div className="card stack"><div className="row-between wrap-mobile"><h3>Người dùng KNTech</h3><span className="badge">Full edit</span></div><div><label className="label">Tìm người dùng</label><input className="input" placeholder="Tìm email, tên, vai trò, lớp..." value={userSearch} onChange={(e) => setUserSearch(e.target.value)} /></div><div className="table-wrap"><table className="table compact-table"><thead><tr><th>Email</th><th>Tên</th><th>Vai trò</th><th>Trạng thái</th><th>Lớp</th></tr></thead><tbody>{filteredUsers.map((user: any) => <tr key={user.id} className={selectedUserId === user.id ? 'row-selected' : ''} onClick={() => setSelectedUserId(user.id)}><td>{user.email}</td><td>{user.tenHienThi || user.hoTen}</td><td>{user.vaiTro}</td><td>{user.trangThai}</td><td>{user.lopHoc || '-'}</td></tr>)}</tbody></table></div></div>
            <div className="card stack"><div className="row-between wrap-mobile"><h3>Tạo người dùng mới</h3><button className="button" onClick={createUser}>Tạo user</button></div><div className="grid-2 compact-filter-grid"><div><label className="label">Email</label><input className="input" placeholder="Email" value={newUser.email} onChange={(e) => setNewUser((prev) => ({ ...prev, email: e.target.value }))} /></div><div><label className="label">Tên hiển thị</label><input className="input" placeholder="Tên hiển thị" value={newUser.tenHienThi} onChange={(e) => setNewUser((prev) => ({ ...prev, tenHienThi: e.target.value }))} /></div><div><label className="label">Họ tên</label><input className="input" placeholder="Họ tên" value={newUser.hoTen} onChange={(e) => setNewUser((prev) => ({ ...prev, hoTen: e.target.value }))} /></div><div><label className="label">Lớp học</label><input className="input" placeholder="Lớp học" value={newUser.lopHoc} onChange={(e) => setNewUser((prev) => ({ ...prev, lopHoc: e.target.value }))} /></div><div><label className="label">Vai trò</label><select className="select" value={newUser.vaiTro} onChange={(e) => setNewUser((prev) => ({ ...prev, vaiTro: e.target.value }))}><option value="HOC_SINH">HOC_SINH</option><option value="GIAO_VIEN">GIAO_VIEN</option><option value="QUAN_TRI_VIEN">QUAN_TRI_VIEN</option><option value="CMS_ROOT">CMS_ROOT</option></select></div><div><label className="label">Trạng thái</label><select className="select" value={newUser.trangThai} onChange={(e) => setNewUser((prev) => ({ ...prev, trangThai: e.target.value }))}><option value="HOAT_DONG">HOAT_DONG</option><option value="KHOA">KHOA</option></select></div></div><div><label className="label">Mật khẩu khởi tạo</label><input className="input" type="password" placeholder="Mật khẩu khởi tạo" value={newUser.matKhauMoi} onChange={(e) => setNewUser((prev) => ({ ...prev, matKhauMoi: e.target.value }))} /></div></div>
          </div>
          <div className="card stack">
            ...
            {selectedUser ? (
              <div className="stack">
                <div className="grid-2 compact-filter-grid"><div><label className="label">Email</label><input className="input" value={selectedUser.email || ''} onChange={(e) => setData((prev: any) => ({ ...prev, users: prev.users.map((u: any) => u.id === selectedUser.id ? { ...u, email: e.target.value } : u) }))} /></div><div><label className="label">Tên hiển thị</label><input className="input" value={selectedUser.tenHienThi || ''} onChange={(e) => setData((prev: any) => ({ ...prev, users: prev.users.map((u: any) => u.id === selectedUser.id ? { ...u, tenHienThi: e.target.value } : u) }))} /></div><div><label className="label">Họ tên</label><input className="input" value={selectedUser.hoTen || ''} onChange={(e) => setData((prev: any) => ({ ...prev, users: prev.users.map((u: any) => u.id === selectedUser.id ? { ...u, hoTen: e.target.value } : u) }))} /></div><div><label className="label">Lớp học</label><input className="input" value={selectedUser.lopHoc || ''} onChange={(e) => setData((prev: any) => ({ ...prev, users: prev.users.map((u: any) => u.id === selectedUser.id ? { ...u, lopHoc: e.target.value } : u) }))} /></div><div><label className="label">Vai trò</label><select className="select" value={selectedUser.vaiTro} onChange={(e) => setData((prev: any) => ({ ...prev, users: prev.users.map((u: any) => u.id === selectedUser.id ? { ...u, vaiTro: e.target.value } : u) }))}><option value="HOC_SINH">HOC_SINH</option><option value="GIAO_VIEN">GIAO_VIEN</option><option value="QUAN_TRI_VIEN">QUAN_TRI_VIEN</option><option value="CMS_ROOT">CMS_ROOT</option></select></div><div><label className="label">Trạng thái</label><select className="select" value={selectedUser.trangThai} onChange={(e) => setData((prev: any) => ({ ...prev, users: prev.users.map((u: any) => u.id === selectedUser.id ? { ...u, trangThai: e.target.value } : u) }))}><option value="HOAT_DONG">HOAT_DONG</option><option value="KHOA">KHOA</option></select></div></div>
                <div className="grid-2 compact-filter-grid"><div><label className="label">Ngày tạo</label><input className="input" value={selectedUser.createdAt ? new Date(selectedUser.createdAt).toLocaleString() : ''} disabled /></div><div><label className="label">Đặt mật khẩu mới</label><input className="input" type="password" placeholder="Để trống nếu không đổi" value={selectedUser.matKhauMoi || ''} onChange={(e) => setData((prev: any) => ({ ...prev, users: prev.users.map((u: any) => u.id === selectedUser.id ? { ...u, matKhauMoi: e.target.value } : u) }))} /></div></div>
      ...
      {tab === 'trang' && (
        <div className="grid-2">
          <div className="card stack"><h3>Trang nội dung</h3><div className="table-wrap"><table className="table compact-table"><thead><tr><th>Tiêu đề</th><th>Slug</th><th>Trạng thái</th></tr></thead><tbody>{data.pages.map((page: any) => <tr key={page.id} onClick={() => setPageForm(page)}><td>{page.tieuDe}</td><td>{page.slug}</td><td>{page.trangThai}</td></tr>)}</tbody></table></div></div>
          <div className="card stack"><h3>Trình biên tập trang</h3><input className="input" placeholder="Tiêu đề" value={pageForm.tieuDe} onChange={(e) => setPageForm((prev) => ({ ...prev, tieuDe: e.target.value }))} /><input className="input" placeholder="Slug" value={pageForm.slug} onChange={(e) => setPageForm((prev) => ({ ...prev, slug: e.target.value }))} /><input className="input" placeholder="Mô tả" value={pageForm.moTa} onChange={(e) => setPageForm((prev) => ({ ...prev, moTa: e.target.value }))} /><textarea className="textarea" placeholder="Nội dung markdown" value={pageForm.noiDungMarkdown} onChange={(e) => setPageForm((prev) => ({ ...prev, noiDungMarkdown: e.target.value }))} /><select className="select" value={pageForm.trangThai} onChange={(e) => setPageForm((prev) => ({ ...prev, trangThai: e.target.value as any }))}><option value="NHAP">Nháp</option><option value="XUAT_BAN">Xuất bản</option></select><button className="button" onClick={savePage}>Lưu trang</button></div>
        </div>
      )}
    </div>
  );
