...
function createDefaultSections() {
  return [
    { tieuDe: 'Khái ni?m tr?ng tâm', mucDo: 'DE', noiDungMarkdown: `## Khái ni?m
- Nêu d?nh nghia c?t lõi.`, thuTu: 1 },
    { tieuDe: 'Công th?c c?t lõi', mucDo: 'TRUNG_BINH', noiDungMarkdown: `## Công th?c
\[A = B\]`, thuTu: 2 },
    { tieuDe: 'Bi?n d?i công th?c', mucDo: 'TRUNG_BINH', noiDungMarkdown: `## Bi?n d?i
1. Thay s?
2. Rút g?n`, thuTu: 3 },
    { tieuDe: 'Ðon v? và d?i lu?ng', mucDo: 'DE', noiDungMarkdown: `## Ðon v?
- Ð?i lu?ng:
- Ðon v? SI:`, thuTu: 4 },
    { tieuDe: 'Ví d? minh h?a', mucDo: 'KHO', noiDungMarkdown: `## Ví d?
- D? ki?n
- Gi?i`, thuTu: 5 }
  ];
}
  ...
  return (
    <div className="stack">
      <div className="grid-2">
        <div>
          <label className="label">Mô t? bài h?c</label>
        ...
        <div className="stack">
          <div>
            ...
            <select className="select" value={selectedType} onChange={(e) => {
              ...
              onChange({ ...lesson, coMoPhong: true, moPhong: { ...(lesson.moPhong || {}), loaiMoPhong: nextType, thamSoJson: preset?.defaultParams || {}, cauHinhJson: { ...config, topic: lesson.chuDeThi || config.topic || 'T?ng h?p', renderer: 'kntech-threejs-full3d', sceneVariant: lesson.slug || lesson.id, allowExternalResources: true } } });
            }}>
              <option value="">Ch?n simulation type</option>
          ...
          <div className="row-between wrap-mobile">
            <label><input type="checkbox" checked readOnly disabled /> B?t bu?c có mô ph?ng riêng cho bài này</label>
            <label><input type="checkbox" checked={lesson.coAI} onChange={(e) => onChange({ ...lesson, coAI: e.target.checked })} /> B?t AI</label>
          </div>
          <div>
            <label className="label">Ch? d? thi / tag renderer</label>
          ...
          <div className="grid-2 compact-filter-grid">
            <div>
              <label className="label">Ch? d? hi?n th?</label>
              <select className="select" value={displayMode} onChange={(e) => onChange({ ...lesson, moPhong: { ...(lesson.moPhong || {}), cauHinhJson: { ...config, displayMode: e.target.value } } })}>
                <option value="simulation">Mô ph?ng tuong tác</option>
                <option value="image">?nh / model tham chi?u</option>
              </select>
            </div>
            <div>
          ...
          <div className="stack">
            ...
            <label className="label">External resources (m?i dòng: Tiêu d?|URL|type|provider)</label>
            ...
            <div className="muted">Cho phép g?n ALL RESOURCES: video, PDF, bài báo, mô hình, web mô ph?ng, slide, ?nh ho?c link h?c li?u m?. Backend s? luu trong lesson config.</div>
          </div>
        </div>
      </div>
      <div className="grid-2">
        <div className="card section-card">
          <h4>Tham s? mô ph?ng</h4>
          <div className="table-wrap">
            <table className="table compact-table">
              <thead><tr><th>Tham s?</th><th>Giá tr?</th></tr></thead>
              <tbody>
                ...
                )) : <tr><td colSpan={2}>KNTech chua có preset tham s? cho bài này.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        <div className="card section-card">
          ...
          <div className="table-wrap">
            <table className="table compact-table">
              <thead><tr><th>Config</th><th>Giá tr?</th></tr></thead>
              <tbody>
...
function LessonContentEditor({ lesson, onChange }: { lesson: any; onChange: (next: any) => void }) {
  ...
  return (
    <div className="stack">
      <div className="row-between wrap-mobile">
        <h4>N?i dung bài gi?ng</h4>
        <button className="button button-secondary" type="button" onClick={() => onChange({ ...lesson, phanKienThuc: [...sections, { tieuDe: 'M?c m?i', mucDo: 'DE', noiDungMarkdown: `## Tiêu d?
- N?i dung`, thuTu: sections.length + 1 }] })}>Thêm m?c n?i dung</button>
      </div>
      {sections.map((section: any, idx: number) => (
        <div key={section.id || idx} className="card section-card stack">
          <div className="grid-3 compact-filter-grid">
                  <input className="input" value={section.tieuDe || ''} onChange={(e) => onChange({ ...lesson, phanKienThuc: sections.map((item: any, i: number) => i === idx ? { ...item, tieuDe: e.target.value } : item) })} placeholder="Tiêu d? m?c" />
            <select className="select" value={section.mucDo || 'DE'} onChange={(e) => onChange({ ...lesson, phanKienThuc: sections.map((item: any, i: number) => i === idx ? { ...item, mucDo: e.target.value } : item) })}>
              <option value="DE">DE</option><option value="TRUNG_BINH">TRUNG_BINH</option><option value="KHO">KHO</option>
                  ...
                  <input className="input" type="number" value={section.thuTu || idx + 1} onChange={(e) => onChange({ ...lesson, phanKienThuc: sections.map((item: any, i: number) => i === idx ? { ...item, thuTu: Number(e.target.value) } : item) })} placeholder="Th? t?" />
          </div>
          ...
          <div className="muted">H? tr? markdown co b?n và công th?c d?ng \(...\) ho?c \[...\]</div>
        </div>
      ))}
    </div>
  );
...
export function AdminPage({ cmsMode = false }: { cmsMode?: boolean }) {
  ...
  async function saveSettings() {
    try {
      ...
      setMessage('KNTech dã luu c?u hình thành công.');
      await load();
  ...
  async function saveLesson() {
    ...
    try {
      ...
      setMessage(`KNTech dã luu bài h?c ${selectedLesson.ten}.`);
      await load();
    } catch (e: any) { setError(e.message); }
  }
  ...
  async function savePage() {
    try {
      ...
      setMessage('KNTech dã luu trang CMS.');
      await load();
    } catch (e: any) { setError(e.message); }
  }
  ...
  async function saveUser() {
    ...
    try {
      ...
      setMessage(`KNTech dã c?p nh?t ngu?i dùng ${selectedUser.email}.`);
      await load();
    } catch (e: any) { setError(e.message); }
  }
  ...
  async function createUser() {
    try {
      ...
      setMessage(`KNTech dã t?o ngu?i dùng ${newUser.email}.`);
  ...
  async function deleteUser() {
    if (!selectedUser) return;
    if (!window.confirm(`Xóa ngu?i dùng ${selectedUser.email}?`)) return;
    try {
      await api.adminDeleteUser(selectedUser.id);
      setMessage(`KNTech dã xóa ngu?i dùng ${selectedUser.email}.`);
      setSelectedUserId('');
      await load();
    } catch (e: any) { setError(e.message); }
  }
  ...
  async function uploadMedia(file?: File | null) {
    ...
    try {
      ...
      setMessage(`Ðã upload ${file.name} vào thu vi?n KNTech.`);
      await load();
  ...
  if (!data) return <div className="card">KNTech dang t?i CMS...</div>;
  ...
  return (
    <div className="stack">
      <div className={cmsMode ? 'card hero-panel cms-hero dashboard-gradient-cms' : 'card hero-panel admin-hero dashboard-gradient-admin'}>
        ...
        <h1 className="page-title">{cmsMode ? 'System Command Center' : 'V?n hành n?i dung và h?c v?'}</h1>
        <p>{cmsMode ? 'CMS Root qu?n lý c?u hình, media, API monitor, system monitor, AI analytics và lesson builder chuyên sâu.' : 'Admin t?p trung vào th?ng kê, bài h?c, ngu?i dùng và ch?t lu?ng v?n hành do CMS Root thi?t l?p.'}</p>
      </div>
        ...
        <div className="stack">
          <div className="grid-2">
            <div className="card stack">
              <div className="row-between"><h3>Lesson coverage theo l?p</h3><span className="badge">KNTech Analytics</span></div>
      ...
      {tab === 'cau-hinh' && (
        <div className="card stack">
          <div className="row-between"><h3>KNTech system settings</h3><button className="button" onClick={saveSettings}>Luu c?u hình</button></div>
      ...
      {tab === 'bai-hoc' && (
        <div className="grid-2">
          <div className="card stack">
            ...
            <div className="grid-2 compact-filter-grid">
              <div><label className="label">Tìm bài h?c</label><input className="input" placeholder="Tìm bài h?c..." value={lessonSearch} onChange={(e) => setLessonSearch(e.target.value)} /></div>
              <div><label className="label">L?c theo ch? d?</label><select className="select" value={lessonGroup} onChange={(e) => setLessonGroup(e.target.value)}><option value="all">T?t c? ch? d?</option>{Array.from(new Set((data.lessons || []).map((item: any) => item.chuDeThi).filter(Boolean))).map((item: any) => <option key={item} value={item}>{item}</option>)}</select></div>
            </div>
            <div className="table-wrap">
              <table className="table compact-table">
                <thead><tr><th>Bài</th><th>Ch? d?</th><th>Tag l?p</th></tr></thead>
                <tbody>
          ...
          <div className="stack">
            <div className="card stack">
              <div className="row-between"><h3>{selectedLesson?.ten || 'Ch?n m?t bài h?c'}</h3><button className="button" onClick={saveLesson}>Luu lesson</button></div>
              {selectedLesson ? <div className="stack"><SimulationEditor lesson={selectedLesson} onChange={(next) => setData((prev: any) => ({ ...prev, lessons: prev.lessons.map((item: any) => item.id === next.id ? next : item) }))} onUploadImage={uploadMediaForLesson} /><LessonContentEditor lesson={selectedLesson} onChange={(next) => setData((prev: any) => ({ ...prev, lessons: prev.lessons.map((item: any) => item.id === next.id ? next : item) }))} /></div> : <div className="note-box">Ch?n m?t bài trong danh sách trái d? biên t?p.</div>}
            </div>
          </div>
        </div>
      )}
      ...
      {tab === 'media' && (
        <div className="grid-2">
          <div className="card stack">
            ...
            <div className="muted">Upload ?nh, video ng?n, mô hình ho?c tài nguyên h? tr? cho lesson builder.</div>
          </div>
          <div className="card stack">
            <h3>T?p g?n dây</h3>
            <div className="table-wrap">
              <table className="table compact-table">
                <thead><tr><th>Tên t?p</th><th>Size</th><th>C?p nh?t</th></tr></thead>
                <tbody>
      ...
      {tab === 'monitor' && (
        <div className="stack">
          ...
          <div className="card stack">
            <h3>System logs</h3>
            <div className="table-wrap"><table className="table compact-table"><thead><tr><th>Th?i gian</th><th>M?c</th><th>Nhóm</th><th>Hành d?ng</th><th>Ð?i tu?ng</th><th>Ngu?i dùng</th></tr></thead><tbody>
      ...
      {tab === 'ai' && (
        <div className="card stack">
          <h3>AI call logs</h3>
          <div className="table-wrap"><table className="table"><thead><tr><th>Provider</th><th>Tác v?</th><th>Prompt</th><th>K?t qu?</th><th>Th?i gian</th></tr></thead><tbody>
      ...
      {tab === 'nguoi-dung' && (
        <div className="grid-2">
          <div className="stack">
            <div className="card stack">
              <div className="row-between wrap-mobile"><h3>Ngu?i dùng KNTech</h3><span className="badge">Full edit</span></div>
              <div><label className="label">Tìm ngu?i dùng</label><input className="input" placeholder="Tìm email, tên, vai trò, l?p..." value={userSearch} onChange={(e) => setUserSearch(e.target.value)} /></div>
              <div className="table-wrap"><table className="table compact-table"><thead><tr><th>Email</th><th>Tên</th><th>Vai trò</th><th>Tr?ng thái</th><th>L?p</th></tr></thead><tbody>
            ...
            <div className="card stack">
              <div className="row-between wrap-mobile"><h3>T?o ngu?i dùng m?i</h3><button className="button" onClick={createUser}>T?o user</button></div>
              <div className="grid-2 compact-filter-grid">
                ...
                <div><label className="label">Tên hi?n th?</label><input className="input" placeholder="Tên hi?n th?" value={newUser.tenHienThi} onChange={(e) => setNewUser((prev) => ({ ...prev, tenHienThi: e.target.value }))} /></div>
                <div><label className="label">H? tên</label><input className="input" placeholder="H? tên" value={newUser.hoTen} onChange={(e) => setNewUser((prev) => ({ ...prev, hoTen: e.target.value }))} /></div>
                <div><label className="label">L?p h?c</label><input className="input" placeholder="L?p h?c" value={newUser.lopHoc} onChange={(e) => setNewUser((prev) => ({ ...prev, lopHoc: e.target.value }))} /></div>
                ...
                <div><label className="label">Tr?ng thái</label><select className="select" value={newUser.trangThai} onChange={(e) => setNewUser((prev) => ({ ...prev, trangThai: e.target.value }))}><option value="HOAT_DONG">HOAT_DONG</option><option value="KHOA">KHOA</option></select></div>
              </div>
              <div><label className="label">M?t kh?u kh?i t?o</label><input className="input" type="password" placeholder="M?t kh?u kh?i t?o" value={newUser.matKhauMoi} onChange={(e) => setNewUser((prev) => ({ ...prev, matKhauMoi: e.target.value }))} /></div>
            </div>
          </div>
          <div className="card stack">
            <div className="row-between wrap-mobile"><h3>{selectedUser?.email || 'Ch?n ngu?i dùng'}</h3><div className="badge-row"><button className="button button-secondary" onClick={saveUser} disabled={!selectedUser}>Luu user</button><button className="button" onClick={deleteUser} disabled={!selectedUser}>Xóa user</button></div></div>
            {selectedUser ? (
              <div className="stack">
                <div className="grid-2 compact-filter-grid">
                  ...
                  <div><label className="label">Tên hi?n th?</label><input className="input" value={selectedUser.tenHienThi || ''} onChange={(e) => setData((prev: any) => ({ ...prev, users: prev.users.map((u: any) => u.id === selectedUser.id ? { ...u, tenHienThi: e.target.value } : u) }))} /></div>
                  <div><label className="label">H? tên</label><input className="input" value={selectedUser.hoTen || ''} onChange={(e) => setData((prev: any) => ({ ...prev, users: prev.users.map((u: any) => u.id === selectedUser.id ? { ...u, hoTen: e.target.value } : u) }))} /></div>
                  <div><label className="label">L?p h?c</label><input className="input" value={selectedUser.lopHoc || ''} onChange={(e) => setData((prev: any) => ({ ...prev, users: prev.users.map((u: any) => u.id === selectedUser.id ? { ...u, lopHoc: e.target.value } : u) }))} /></div>
                  ...
                  <div><label className="label">Tr?ng thái</label><select className="select" value={selectedUser.trangThai} onChange={(e) => setData((prev: any) => ({ ...prev, users: prev.users.map((u: any) => u.id === selectedUser.id ? { ...u, trangThai: e.target.value } : u) }))}><option value="HOAT_DONG">HOAT_DONG</option><option value="KHOA">KHOA</option></select></div>
                </div>
                <div className="grid-2 compact-filter-grid">
                  <div><label className="label">Ngày t?o</label><input className="input" value={selectedUser.createdAt ? new Date(selectedUser.createdAt).toLocaleString() : ''} disabled /></div>
                  <div><label className="label">Ð?t m?t kh?u m?i</label><input className="input" type="password" placeholder="Ð? tr?ng n?u không d?i" value={selectedUser.matKhauMoi || ''} onChange={(e) => setData((prev: any) => ({ ...prev, users: prev.users.map((u: any) => u.id === selectedUser.id ? { ...u, matKhauMoi: e.target.value } : u) }))} /></div>
                </div>
                <div className="note-box">CMS Root / Admin có th? t?o m?i, c?p nh?t email, tên, vai trò, tr?ng thái, l?p h?c và reset m?t kh?u ngay t?i dây.</div>
              </div>
            ) : <div className="note-box">Ch?n ngu?i dùng t? b?ng bên trái d? biên t?p d?y d?.</div>}
          </div>
        </div>
      )}
      ...
      {tab === 'trang' && (
        <div className="grid-2">
          <div className="card stack">
            <h3>Trang n?i dung</h3>
            <div className="table-wrap"><table className="table compact-table"><thead><tr><th>Tiêu d?</th><th>Slug</th><th>Tr?ng thái</th></tr></thead><tbody>
          ...
          <div className="card stack">
            <h3>Trình biên t?p trang</h3>
            <input className="input" placeholder="Tiêu d?" value={pageForm.tieuDe} onChange={(e) => setPageForm((prev) => ({ ...prev, tieuDe: e.target.value }))} />
            <input className="input" placeholder="Slug" value={pageForm.slug} onChange={(e) => setPageForm((prev) => ({ ...prev, slug: e.target.value }))} />
            <input className="input" placeholder="Mô t?" value={pageForm.moTa} onChange={(e) => setPageForm((prev) => ({ ...prev, moTa: e.target.value }))} />
            <textarea className="textarea" placeholder="N?i dung markdown" value={pageForm.noiDungMarkdown} onChange={(e) => setPageForm((prev) => ({ ...prev, noiDungMarkdown: e.target.value }))} />
            <select className="select" value={pageForm.trangThai} onChange={(e) => setPageForm((prev) => ({ ...prev, trangThai: e.target.value as any }))}><option value="NHAP">Nháp</option><option value="XUAT_BAN">Xu?t b?n</option></select>
            <button className="button" onClick={savePage}>Luu trang</button>
          </div>
        </div>
      )}
    </div>
