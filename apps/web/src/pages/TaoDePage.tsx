...
type ExamDraft = {
  ...
  attempts: any[];
  antiCheat: { maxTabSwitch: number; daoCauHoi: boolean; fullScreenRequired: boolean; strictAntiCheat: boolean; hideResultDetails: boolean };
};

function statusColor(status: string) {
  if (status === 'STARTED') return 'success';
  if (status === 'STOPPED' || status === 'LOCKED') return 'danger';
...
function cleanText(value: unknown, fallback = '') {
  const cleaned = repairVietnameseText(value);
  return cleaned || fallback;
}

export function TaoDePage() {
  const [curriculum, setCurriculum] = useState<any[]>([]);
  const [form, setForm] = useState({ ten: 'Kiểm tra Vật lý', lop: 12, thoiGianPhut: 15, soLuongCau: 5, mucDo: 'TRUNG_BINH', baiHocSlug: '', daoCauHoi: true, fullScreenRequired: true, strictAntiCheat: true, hideResultDetails: true });
  ...
  useEffect(() => {
    if (!form.baiHocSlug && lessons[0]?.slug) setForm((prev) => ({ ...prev, baiHocSlug: lessons[0].slug }));
  }, [lessons.length]);
          ...
          questions: Array.isArray(item?.questions)
            ? item.questions.map((q: any) => ({
                ...
                options: Array.isArray(q?.options)
                  ? q.options.map((opt: any) => ({ ...opt, text: cleanText(opt?.text) }))
                  : []
              }))
            : item?.questions
        }))
      : [];
    setExams(sanitizedList);
    if (focusId) {
  ...
  function mapToDraft(raw: any): ExamDraft {
    return {
      ...
      questions: (raw.questions || []).map((q: any, i: number) => ({
        ...
        options: (q.options || []).map((opt: any, oi: number) => ({ key: opt.key || defaultOptionKey(oi), text: cleanText(opt.text) })),
        dapAnDungJson: (q.dapAnDungJson || q.correctAnswers || []).map((item: any) => cleanText(item)),
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const mapped = mapToDraft(res.de);
      setDraft(mapped);
      setStep('edit');
  ...
  return (
    <div className="stack">
      <div className="card hero-panel dashboard-gradient-admin">
        ...
        <p>AI tự sinh câu hỏi → Giáo viên chỉnh sửa → Confirm → Start. Học sinh quét QR vào phòng thi.</p>
      </div>

      {error && <div className="error-box">{error}</div>}

      <div className="grid-2 lesson-layout">
        {/* LEFT: Create form + exam list */}
        <div className="stack">
          {step === 'form' && (
            <div className="card stack">
              ...
              <div className="exam-create-row exam-create-row--toggles">
                ...
                <label className="toggle-chip"><input type="checkbox" checked={form.fullScreenRequired} onChange={(e) => setForm({ ...form, fullScreenRequired: e.target.checked })} /><span>Fullscreen</span></label>
                <label className="toggle-chip"><input type="checkbox" checked={form.strictAntiCheat} onChange={(e) => setForm({ ...form, strictAntiCheat: e.target.checked })} /><span>Chống gian lận</span></label>
              ...
              <LoadingButton
                ...
                loading={creating}
                loadingText="AI đang sinh câu hỏi..."
              >
                🤖 Tạo đề bằng AI
              </LoadingButton>
            </div>
          ...
          <div className="card stack">
            <div className="row-between wrap-mobile">
              ...
              {step !== 'form' && (
                <LoadingButton className="button-secondary" onClick={() => { setDraft(null); setStep('form'); }}>+ Tạo đề mới</LoadingButton>
              )}
            </div>
            {exams.length === 0 ? (
              <div className="note-box">Chưa có đề nào. Tạo đề đầu tiên ở trên.</div>
            ) : (
        ...
        {/* RIGHT: Edit / control panel */}
        <div className="stack">
          {!draft && (
            <div className="card note-box" style={{ textAlign: 'center', padding: '48px 24px' }}>
              <div style={{ fontSize: 48, marginBottom: 12 }}>📝</div>
              <p>Sau khi AI tạo xong, đề sẽ hiện ở đây để bạn <strong>chỉnh sửa từng câu</strong> rồi mới Confirm và Start.</p>
            </div>
          )}
          ...
          {draft && (
            <div className="card stack">
              ...
              <div className="exam-action-row">
                {draft.status === 'DRAFT' && (
                  <LoadingButton
                    ...
                    loading={confirming}
                    loadingText="Đang lưu & confirm..."
                  >
                    ✅ Lưu & Confirm
                  </LoadingButton>
                )}
                {draft.status === 'CONFIRMED' && (
                  <LoadingButton onClick={() => handleAction('start')} loading={actionLoading === 'start'} loadingText="Đang start...">▶ Start đề</LoadingButton>
                )}
                {draft.status === 'STARTED' && (
                  <LoadingButton className="button-secondary" onClick={() => handleAction('stop')} loading={actionLoading === 'stop'} loadingText="Đang dừng...">⏹ Dừng đề</LoadingButton>
                )}
                {(draft.status === 'STOPPED' || draft.status === 'CONFIRMED') && (
                  <LoadingButton onClick={() => handleAction('start')} loading={actionLoading === 'start'} loadingText="Đang start...">▶ Start lại</LoadingButton>
                )}
                {draft.status !== 'STARTED' && (
                  <>
                    <LoadingButton className="button-secondary" onClick={() => handleAction('lock')} loading={actionLoading === 'lock'} loadingText="Đang khóa...">🔒 Khóa</LoadingButton>
                    <LoadingButton className="button-secondary" style={{ color: '#ef4444' }} onClick={() => handleAction('delete')} loading={actionLoading === 'delete'} loadingText="Đang xóa...">🗑 Xóa</LoadingButton>
                  </>
                )}
                <LoadingButton
              ...
              {/* Question editor — row layout */}
              {draft.status === 'DRAFT' && (
                <div className="stack">
                  <div className="row-between wrap-mobile">
                    <h4>Chỉnh sửa câu hỏi</h4>
                    <span className="badge badge-variant-1">{draft.questions.length} câu · AI đã sinh</span>
                  </div>
                  <div className="note-box">Sửa trực tiếp nội dung, đáp án ABCD và lời giải. Nhấn <strong>Lưu & Confirm</strong> ở trên khi hoàn tất.</div>

                  {draft.questions.map((q, qi) => (
                    <div key={q.id} className="exam-question-editor card">
                      {/* Row 1: Question number + level + content */}
                      <div className="exam-q-header">
                        ...
                        <textarea
                          className="textarea exam-q-content"
                          value={q.noiDung}
                          rows={2}
                          onChange={(e) => updateQuestion(q.id, { noiDung: e.target.value })}
                          placeholder="Nội dung câu hỏi..."
                        />
                      </div>

                      {/* Row 2: Options A B C D inline */}
                      <div className="exam-options-row">
                        {q.options.map((opt, oi) => (
                          <div key={`${q.id}-${oi}`} className="exam-option-inline">
                            ...
                            <input
                              className="input exam-option-text"
                              value={opt.text}
                      ...
                      {/* Row 3: Correct answer + explanation */}
                      <div className="exam-q-footer">
                        <div className="exam-q-footer-field">
                          <label className="label">✅ Đáp án đúng</label>
                          <input
                            className="input"
                            value={(q.dapAnDungJson || []).join(', ')}
                            onChange={(e) => updateQuestion(q.id, { dapAnDungJson: e.target.value.split(',').map((x) => x.trim().toUpperCase()).filter(Boolean) })}
                            placeholder="VD: A hoặc A, C"
                          />
                        </div>
                        <div className="exam-q-footer-field exam-q-footer-field--wide">
                          <label className="label">💬 Lời giải / Giải thích</label>
                          <textarea
                            className="textarea"
                            rows={2}
                            value={q.giaiThich}
                            onChange={(e) => updateQuestion(q.id, { giaiThich: e.target.value })}
              ...
              {/* Preview table when confirmed/started */}
              {draft.status !== 'DRAFT' && (
                <div className="table-wrap">
                  <table className="table compact-table">
                    <thead>
                    ...
                    <tbody>
                      {draft.questions.map((q, i) => (
                        <tr key={q.id}>
                          <td>{i + 1}</td>
                          <td style={{ maxWidth: 180 }}>{q.noiDung}</td>
                          {q.options.map((opt) => (
                            <td key={opt.key} style={{ color: q.dapAnDungJson.includes(opt.key) ? '#16a34a' : undefined, fontWeight: q.dapAnDungJson.includes(opt.key) ? 700 : undefined, maxWidth: 120 }}>
                              {opt.text}
                            </td>
                          ))}
                          {q.options.length < 4 && Array.from({ length: 4 - q.options.length }).map((_, i) => <td key={`empty-${i}`}>–</td>)}
                  ...
                  <>
                    <div className="table-wrap">
                      <table className="table compact-table">
                        ...
                        <tbody>
                          {draft.attempts.map((attempt: any) => (
                            <tr key={attempt.id}>
                      ...
                      <div key={`${attempt.id}-detail`} className="note-box">
                        ...
                        {Array.isArray(attempt.integrityEvents) && attempt.integrityEvents.length ? (
                          <div style={{ marginTop: 6 }}>
                            {attempt.integrityEvents.map((event: any, index: number) => (
